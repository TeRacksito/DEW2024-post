...
test("Create Post, attach a user and publish", () => {
  ...
  expect(post.publish()).toContain(
    "Pepe is reading Total JS publication number 1",
  );
});
...
test("Create Post: 1. Publish. 2- Attach one users and publish. 3- Attach a client and publish", () => {
  ...
  expect(reading).not.toContain(
    "Mary is reading Total JS publication number 2",
  );
...
test("Create Magazine: 1. Publish. 2- Attach two clients and publish. 3- Attach a user and publish. 4- Detach a client and publish", () => {
  ...
  expect(reading).toContain(
    "Pepe is reading Total JS publication number 2 of Programming",
  );
  expect(reading).toContain(
    "Mary is reading Total JS publication number 2 of Programming",
  );
  const lucy = new User("Lucy");
  post.attach(lucy);
  reading = post.publish();
  expect(reading).toContain(
    "Pepe is reading Total JS publication number 3 of Programming",
  );
  expect(reading).toContain(
    "Mary is reading Total JS publication number 3 of Programming",
  );
  expect(reading).not.toContain(
    "Lucy is reading Total JS publication number 3 of Programming",
  );
  post.detach(pepe);
  reading = post.publish();
  expect(reading).not.toContain(
    "Pepe is reading Total JS publication number 4 of Programming",
  );
  expect(reading).toContain(
    "Mary is reading Total JS publication number 4 of Programming",
  );
});
